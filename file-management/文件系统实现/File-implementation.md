...
![image](https://github.com/YC-L/Postgraduate-examination/blob/Operating-System/imgs/FAT-table-structure.png)
...
- 当内存中栈的结点全部出栈以后，将最后一个出栈的结点的块中的栈保存到内存(超级块)中，更新指针和栈中元素数量n
...
- 从超级块中分配28、26、32、29、22、23，将23号盘块空闲盘块内容存储到超级块中
...
- 之后用户删除文件B，文件B占用的盘块号依次为51、52、53、54、55，画出系统回收文件B占用的空间

- 一共有5个块需要回收

- 回收51、52，超级块栈满，分出一个空闲盘块将超级块内容复制到该空闲盘块中
