...
- 随机探测再散列、二次探测 -1/αln(1-α)

- 链地址，1 + 1/α

#### 查找失败

- 线性探测再散列，1/2(1 + 1/(1-α)^2)
...
- 设散列表有m个存储单元，散列函数H(key)=key%p，则p最好选择，**小于等于m的最大质数**(素数)

- 为了有效德应用HASH查找技术，必须解决的两个问题是**构造一个好的HASH函数**和**让值均匀分布于HASH表**

- 装填因子的值越大，存取元素时发生冲突的可能性就越大 

### 典型例题

- 将关键字序列（7，8，30，11，18，9，14）散列存储到散列表中

- 散列表的存储空间是一个下标从0开始的一维数组，散列函数为：H(key)=(key×3) MOD 7

- 处理冲突采用线性探测再散列法，要求装填（载）因子为0.7

- 请画出所构造的散列表

- 装填因子 α=n/m=7/m=0.7，m=10，表长度为10
...
- 分别计算等概率情况下查找成功和不成功的平均查找长度

- ASL成功 = (1\*4 + 2\*1 + 3\*2)/7 = 12/7

- ASL失败 = (3 + 2 + 1 + 2 + 1 + 5 + 4)/7 = 18/7

- 不成功时，求各位置对应探测次数，H(key)=3key % 7  值0-6
